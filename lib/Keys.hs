...
dmenuLikeXPConfig = def { font            = "xft:Terminus-10"
                        , bgColor         = "#1C1C1C"
                        , fgColor         = "#92cced"
                        , bgHLight        = "#f17413"
                        , fgHLight        = "#18001B"
...
xmobarLikeXPConfig =
    dmenuLikeXPConfig { bgColor = "#1C1C1C"
                      , fgColor = "#ca8f2d"
                      }
...
restartXmonad = do
    ...
    spawn ("if type xmonad;"
        `mappend` "then xmonad --recompile && xmonad --restart;"
        `mappend` "else xmessage xmonad not in \\$PATH: \"$PATH\";"
        `mappend` "fi")

curLayout :: X String
curLayout = fmap
    (description . W.layout . W.workspace . W.current)
    (gets windowset)
...
moeKeysP conf =
    ...
    , (otherModMask, action) <- [ ("",   windows . W.greedyView)
                                , ("S-", windows . W.shift)]
    ]
